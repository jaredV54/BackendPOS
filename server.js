...
const pool = mysql.createPool({
  host:  'https://backendpos-production.up.railway.app',
  user: 'root',
  port: '3306',
  password: '4hq183kl',
  database: 'POS',
    ...
    if (emailData.length > 0) {
      pool.query(handlePassword, [req.body.email, req.body.password], (err, passwordData) => {
        ...
        if (passwordData.length > 0) {
          return res.json({ message: "Requirements Matched", name: passwordData[0].name });
        } else {
...
app.post('/userTypeAndStore', (req, res) => {
  const userTypeAndStoreQuery = "SELECT userType, storeId FROM login WHERE `email` = ? AND `password` = ?";
